...
def Get_Twitter_Data(**kwargs):
    ...
    for i in tweepy.Cursor(api.search_tweets, q = 'Bitcoin -filter:retweets OR BTC -filter:retweets').items(150):
...
def load_data_to_DB(**kwargs):
    ...
    df.to_sql('twitter_btc', engine, if_exists='append', index=False)
